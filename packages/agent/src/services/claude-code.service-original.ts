...
export class ClaudeCodeService extends EventEmitter {
  ...
  private async executeInternal(config: {
    prompt: string;
    sessionId?: string;
    ...
    try {
      console.log(`🤖 Executing Claude Code command: "${prompt}"${sessionId && continueSession ? ` (session: ${sessionId})` : sessionId ? ` (new session: ${sessionId})` : ' (no session)'}`)
      ...
      // Session handling logic (shared by all execute methods)
      if (claudeSessionId) {
      ...
      // Add all API options from the client
      Object.assign(queryOptions, apiOptions)
      ...
      // Save session if sessionId provided and sessionFile exists
